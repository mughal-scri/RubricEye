...
import { createQuestionGroup, deleteQuestionGroup, listQuestionBank, listQuestionGroups, QuestionBankItem, QuestionGroup } from "../api/client";
...
export default function QuestionGroupSetup() {
  ...
  return <div>
    ...
    <div className="page-header"><div className="page-title-group"><div className="eyebrow">Assessment setup</div><h1>Question groups</h1><p>Define which questions are compulsory and which follow a choose-N-of-M rule.</p></div></div>
    ...
    <div className="info-panel"><Info size={17} /><div><strong>How choice questions work</strong><p>For a choose-N-of-M group, the first N attempted items in ascending question order are scored. A crossed-out answer counts as an attempt and consumes a slot.</p></div></div>
    ...
    <div className="section-heading"><div><h2>Existing groups</h2><p>Ungrouped questions are treated as compulsory by default during grading.</p></div></div>
    {groups.length === 0 ? <div className="empty-state"><h3>No question groups configured</h3><p>Add a compulsory group or choose-N-of-M group if the paper needs one.</p></div> : <div className="card-grid">{groups.map((group) => <article className="card group-card" key={group.id}><div className="group-card-header"><div><h3>{group.group_name}</h3><span className={`badge ${group.selection_type === "compulsory" ? "badge-slate" : "badge-indigo"}`}>{group.selection_type === "compulsory" ? "Compulsory" : `Choose ${group.n_required} of ${group.selection_units?.length ?? group.question_numbers.length}`}</span></div><button type="button" className="icon-button danger" onClick={() => removeGroup(group)} disabled={deletingId === group.id} aria-label={`Delete ${group.group_name}`}><Trash2 size={15} /></button></div><p className="group-question-list">{group.selection_type === "choose_n_of_m" ? "Selectable choices" : "Questions"}: {(group.selection_units?.length ? group.selection_units : group.question_numbers.map((question) => [question])).map((unit) => unit.join(" + ")).join(", ")}</p>{group.selection_type === "choose_n_of_m" && <p className="field-help">First {group.n_required} attempted items count in ascending order.</p>}</article>)}</div>}
  </div>;
}
