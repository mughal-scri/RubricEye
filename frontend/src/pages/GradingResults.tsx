  ...
  return <div>
    ...
    <div className="review-layout"><aside className="review-queue" aria-label="Questions to review"><div className="queue-header"><strong>Review queue</strong><span>{remainingCount} open</span></div>{data.results.map((result) => { const isActive = result.question_number === selectedQuestion; const closed = isClosed(result); const issue = result.truncation_flag ? "Possible crop overflow" : result.choice_status !== "graded" ? choiceStatusLabel(result.choice_status) : result.flags[0]; return <button key={result.question_number} type="button" className={`queue-item ${isActive ? "is-active" : ""} ${closed ? "is-closed" : ""}`} onClick={() => setSelectedQuestion(result.question_number)} aria-current={isActive ? "true" : undefined}><span className="queue-item-top"><strong>Q{result.question_number}</strong>{closed ? <span className="badge badge-slate">Closed</span> : result.reviewed ? <span className="badge badge-success"><CheckCircle2 size={11} /> Confirmed</span> : <span className="badge badge-warning"><Circle size={11} /> Needs review</span>}</span><span className="queue-item-bottom">{result.ai_score !== null ? `${result.reviewed ? result.human_confirmed_score : result.ai_score} / ${result.ai_total_possible ?? "?"}` : issue ?? "No draft score"}</span></button>; })}</aside>
      ...
      {selected ? <section className="evidence-workspace" aria-labelledby="selected-question-title"><div className="evidence-header"><div><span className="eyebrow">Question {selected.question_number}</span><h2 id="selected-question-title">Evidence review</h2></div><div className="button-row"><span className={`badge ${selected.reviewed ? "badge-success" : isClosed(selected) ? "badge-slate" : "badge-warning"}`}>{selected.reviewed ? "Examiner confirmed" : isClosed(selected) ? "Closed by local rule" : "Draft · review required"}</span></div></div>
        ...
        <div className="evidence-grid"><div className="evidence-pane"><div className="panel-heading"><div><h3>Evidence used for this draft</h3><p>Click an image to inspect it without leaving the review.</p></div></div>{selected.region_preview_urls.length === 0 ? <div className="no-preview"><ImageOff size={18} /><span>No crop preview is available. Inspect the answer sheet directly before confirming.</span></div> : <div className="crop-grid">{selected.region_preview_urls.map((url, index) => <button type="button" className="crop-button" key={url} onClick={() => setSelectedPreviewUrl(url)}><img src={fileUrl(url)} alt={`Answer evidence for question ${selected.question_number}, region ${index + 1}`} /><span>Region {index + 1} · <ZoomIn size={13} /> Enlarge</span></button>)}</div>}</div><div className="context-pane"><div className="panel-heading"><div><h3>Question context</h3><p>Use the approved criteria when available.</p></div></div><div className="context-placeholder">Question text is not included in this result payload. Review the approved question bank if more context is needed.</div><div className="signal-list"><div><span>AI recommendation</span><strong className="text-ai">{isClosed(selected) ? "No model score" : selected.choice_status === "flagged_ambiguous" ? "Manual decision" : "Draft only"}</strong></div><div><span>Model confidence</span><strong>{selected.confidence}</strong></div><div><span>Answer state</span><strong>{selected.ink_status}</strong></div><div><span>Choice rule</span><strong>{choiceStatusLabel(selected.choice_status)}</strong></div>{selected.ink_density_ratio !== null && <div><span>Ink density</span><strong>{selected.ink_density_ratio.toFixed(3)}</strong></div>}</div></div></div>
